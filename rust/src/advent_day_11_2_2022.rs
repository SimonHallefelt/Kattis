...
fn main() {
    let mut items: Vec<Vec<i128>> = Vec::new();
    let mut operation: Vec<Vec<String>> = Vec::new();
    let mut test: Vec<Vec<i128>> = Vec::new();
    let mut inspected_items: Vec<i128> = Vec::new();
    let monkys = 8;
    let rounds = 10000;
    let mut pruduct_all_divisors: i128 = 1;
    ...
    for i in 0..monkys {
        ...
        for j in 2..input1.len() {
            items_temp.push(input1[j].to_string().parse::<i128>().unwrap());
        }
        ...
        let test_temp = vec![input3[3].to_string().parse::<i128>().unwrap(), input4[5].to_string().parse::<i128>().unwrap(), input5[5].to_string().parse::<i128>().unwrap()];
        test.push(test_temp);
    ...
    for r in 0..rounds {
        for i in 0..monkys {
            ...
            for _ in 0..items[i].len() {
                let mut item: i128 = items[i].remove(0);
                if operation[i][0] == "+" {
                    if operation[i][1] == "old" {
                        item += item;
                    }else{
                        item += operation[i][1].to_string().parse::<i128>().unwrap();
                    }
                }else{
                    if operation[i][1] == "old" {
                        item *= item;
