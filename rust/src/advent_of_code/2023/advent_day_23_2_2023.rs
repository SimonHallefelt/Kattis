...
fn main(){
    ...
    // let file_path = "src\\advent_of_code\\2023\\data\\day_23_test.txt"; // 154
    let file_path = "src\\advent_of_code\\2023\\data\\day_23.txt"; // 6406
...
fn run(graph: &HashMap<(usize,usize), Node>, start: (usize,usize), end: (usize,usize)) -> i64 {
    ...
    while !walking.is_empty() {
        ...
        for n in &graph.get(&w.pos).unwrap().go_to {
...
fn build_graph(map: &Vec<Vec<char>>, start: (usize,usize), end: (usize,usize)) -> HashMap<(usize,usize), Node> {
    ...
    while !nodes_left.is_empty() {
        ...
        let mut b = false;
        let mut the_end = (0, (0,0));
        for i in can_go_to.clone() {
            if i.1 == end {
                b = true;
                the_end = i;
            }
        }
        if b {graph.get_mut(&node).unwrap().go_to = Vec::from([the_end]);}
        else {graph.get_mut(&node).unwrap().go_to = can_go_to.clone();}
