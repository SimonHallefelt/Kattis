...
fn main() {
    ...
    //let file_path = "src\\advent_of_code\\2023\\data\\day_17_test.txt"; //94
    //let file_path = "src\\advent_of_code\\2023\\data\\day_17_test2.txt"; //71
    let file_path = "src\\advent_of_code\\2023\\data\\day_17.txt"; //1283
...
fn run(map: Vec<Vec<i64>>) -> i64 {
    ...
    walking.push(Node {cost: 0, pos: (0,0), dir: (1,0), same_dir: -1, path: Vec::new()});
    walking.push(Node {cost: 0, pos: (0,0), dir: (0,1), same_dir: -1, path: Vec::new()});

    while let Some(Node {cost, pos, dir, same_dir, path}) = walking.pop() {
        if same_dir == 10 {continue;}
        if !visited.insert((pos, dir, same_dir)) {continue;}
        //println!("hej, cost = {}, pos = {:?}", cost, pos);
        if pos.0+1 == map.len() as i64 && pos.1+1 == map[0].len() as i64 && same_dir >= 3 {t = cost; final_path = path.clone(); break;}
