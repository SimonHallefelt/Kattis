...
fn run(file_path: &str) -> u32 {
    ...
    for z in zeros {
        ...
        find_paths(&map, z.0+1, z.1, 0, &mut nines);
        find_paths(&map, z.0-1, z.1, 0, &mut nines);
        find_paths(&map, z.0, z.1+1, 0, &mut nines);
        find_paths(&map, z.0, z.1-1, 0, &mut nines);
        tot += nines.len() as u32;
    }

    tot
}

fn find_paths(map: &Vec<Vec<u32>>, y: i32, x: i32, current: u32, nines: &mut HashSet<(i32, i32)>) {
    ...
    if v == current +1 {
        if v == 9 {
            ...
            find_paths(map, y+1, x, v, nines);
            find_paths(map, y-1, x, v, nines);
            find_paths(map, y, x+1, v, nines);
            find_paths(map, y, x-1, v, nines);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_data() {
        assert_eq!(36, run("src\\advent_of_code\\2024\\data\\day_10_test.txt"))
    }

    #[test]
    fn actual_data() {
        assert_eq!(778, run("src\\advent_of_code\\2024\\data\\day_10.txt"))
    }
}